      ...
      <div className="w-full h-full overflow-x-scroll scroll whitespace-nowrap scroll-smooth flex">
        {data.map((item) => {
          ...
          return (
            <>
              <div
                className="w-[900px] h-[450px] flex justify-center flex-col gap-y-1 mr-10 bg-cover bg-left pr-96 rounded-2xl"
                key={item.id}
                style={{
                  backgroundImage: `url('https://image.tmdb.org/t/p/w500${item.backdrop_path}')`,
                }}
              >
                <div className="flex flex-col justify-center gap-y-1 bg-gradient-to-r from-gray-800 text-white h-full w-full pl-10 pr-[860px] rounded-tl-2xl rounded-bl-2xl">
                  <Link to={`/${item.id}`}>
